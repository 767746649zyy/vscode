...
import {
	...
	copyCopilotNativeDeps,
	prepareBuiltInCopilotExtensionShims,
} from './lib/copilot.ts';
...
function packageTask(
	...
	const task = () => {
		...
		const isInsiderOrExploration =
			quality === 'insider' || quality === 'exploration';
		const embedded = isInsiderOrExploration
			? (product as typeof product & { embedded?: EmbeddedProductInfo })
					.embedded
			: undefined;
		...
		if (platform === 'win32') {
			...
			if (embedded) {
				all = es.merge(
					all,
					gulp.src('resources/win32/sessions.ico', { base: '.' }),
				);
			}
			...
			.pipe(
				filter(
					[
						...
						'!LICENSE',
						'!version',
						...(platform === 'darwin' && !isInsiderOrExploration
							? ['!**/Contents/Applications', '!**/Contents/Applications/**']
							: []),
						...(platform === 'win32' && !isInsiderOrExploration
							? ['!**/electron_proxy.exe']
							: []),
					],
					{ dot: true },
...
function copyCopilotNativeDepsTask(
	platform: string,
	arch: string,
	destinationFolderName: string,
) {
...
const innoSetupConfig: Record<
	string,
	{ codePage: string; defaultInfo?: { name: string; id: string } }
> = {
	'zh-cn': {
		codePage: 'CP936',
